...
class Matrix:
    ...
    def __mul__(self, other: Matrix | Vector | float) -> Matrix | Vector:  # Overload
        if (isinstance(other, Matrix)):
            return self.__mul__Matrix(other)
    ...
    def round(self, value: float) -> Matrix:
        ...
        for iRowIndex in range(self.iRows):
            for iColumnIndex in range(self.iColumns):
                newMatrix[iRowIndex][iColumnIndex] = round(newMatrix[iRowIndex][iColumnIndex],
                                                           value)  # ! Il faut demander si nous avons le droit d'utiliser la fonction round()
        return newMatrix
    ...
    def com(self) -> Matrix:
        ...
        for iRowIndex in range(self.iRows):
            for iColumnIndex in range(self.iColumns):
                newMatrix[iRowIndex][iColumnIndex] = (-1) ** (iRowIndex + iColumnIndex) * self.remove(iRowIndex,
                                                                                                      iColumnIndex).det()
        return newMatrix
